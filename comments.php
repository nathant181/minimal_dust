			...
			<?php foreach ( $post->comments->moderated as $comment ) : ?>
				...
				<div id="comment-<?php echo $comment->id; ?>" class="post-comment">
					<div class="post-comment-commentor">
						<h2><?php echo $comment_url; ?></h2>
				    		<div class="post-comment-body">
				    			<?php echo $comment->content_out; ?>
				   	 		<p class="post-comment-link">
				   	 			<a href="#comment-<?php echo $comment->id; ?>" title="<?php _e( 'Time of this comment - Click for comment permalink' ); ?>"><?php $comment->date->out(); ?></a>
				   	 		</p>

							<?php if ( $comment->status == Comment::STATUS_UNAPPROVED ) : ?>
								<p class="comment-message"><em><?php _e( 'Your comment is awaiting moderation' ) ;?></em>
								</p>
						 	<?php endif; ?>
